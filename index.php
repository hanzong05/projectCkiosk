...
?>
<section id="announcement" class="content-section bg-light remove-padding-sm py-5">
    ...
    <div id="announcement-container" class="announcement-container mx-auto bg-gray-50 p-4 bg-[#2e0f13] rounded-md shadow">
    ...
    <div class="announcement-header flex items-center mb-4">
        ...
        <div class="ml-6">
            <h3 class="font-semibold text-lg text-gray-800" data-original-text="<?= htmlspecialchars($announcement['org_name']) ?>"><?= htmlspecialchars($announcement['org_name']) ?></h3>
            <p class="text-sm text-gray-500">Posted by <?= htmlspecialchars($announcement['creator_name']) ?></p>
        </div>
    </div>
                        
                        <!-- Announcement Details (Ensure it stays within the section) -->
                        <div class="announcement-details" data-original-text="<?= htmlspecialchars($announcement['announcement_details']) ?>">
        <?= nl2br($announcement['announcement_details']) ?>
                        ...
                        <div class="announcement-footer flex justify-between mt-4">
...
<div id="imageModal" class="modal" style="display: none;">
    ...
    <img class="modal-img w-full max-w-3xl mx-auto" src="" alt="Image preview">
    ...
    if (prevScrollpos > currentScrollPos) {
        ...
        document.querySelector("nav").style.top = "-70px"; // Hide navbar (adjust based on navbar height)
    }
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        timeElements.forEach(function(timeElement) {
            ...
            console.log("Data created-at:", createdAt);  // Log to see the actual value
            
            if (createdAt) {
                timeElement.textContent = formatRelativeTime(createdAt);
            } else {
                timeElement.textContent = "Date not available";  // Fallback message
            }
            
    function formatRelativeTime(dateString) {
        const now = new Date();
        const date = new Date(dateString); // Date object from ISO 8601 string
    ...
    function filterAnnouncements() {
        ...
        announcements.forEach(announcement => {
            const announcementDetails = announcement.querySelector('.announcement-details');
            const orgName = announcement.querySelector('.font-semibold');
          ...
          // Category filter
            if (selectedCategory && selectedCategory !== 'all') {
                ...
                isMatch = announcementCategory && announcementCategory.toLowerCase() === selectedCategory.toLowerCase();
            }
                        
                        // Date filter
            if (isMatch && createdAt && (fromDate || toDate)) {
                if (fromDate && createdAt < fromDate) isMatch = false;
                if (toDate && createdAt > toDate) isMatch = false;
            }
            ...
            if (isMatch && searchTerm) {
                ...
                if (announcementDetails) {
                    const originalText = announcementDetails.getAttribute('data-original-text') || 
                                       announcementDetails.innerText;
                    if (originalText.toLowerCase().includes(searchTerm)) {
                        isMatch = true;
                        announcementDetails.innerHTML = highlightText(originalText, searchTerm);
                    }
                }
                
                if (orgName) {
                    const originalOrgName = orgName.getAttribute('data-original-text') || 
                                         orgName.innerText;
                    if (originalOrgName.toLowerCase().includes(searchTerm)) {
                        isMatch = true;
                        orgName.innerHTML = highlightText(originalOrgName, searchTerm);
                    }
                }
            }
            ...
            if (!noResultsMessage) {
                ...
                noResultsMessage.innerHTML = `
                    <div class="custom-alert">
                        <i class="fas fa-search me-2" style="font-size: 2rem;"></i>
                        <p class="h4 mb-0">No announcements found matching your filters</p>
                    </div>
                `;
                document.getElementById('announcement-container').appendChild(noResultsMessage);
            }
    ...
    function clearAllFilters() {
        ...
        announcements.forEach(announcement => {
            ...
            if (announcementDetails) {
                const originalText = announcementDetails.getAttribute('data-original-text');
                if (originalText) {
                    announcementDetails.textContent = originalText;
                }
            }
            ...
            if (orgName) {
                const originalOrgName = orgName.getAttribute('data-original-text');
                if (originalOrgName) {
                    orgName.textContent = originalOrgName;
                }
            }
